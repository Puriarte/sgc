...
@NamedQueries({
        ...
        @NamedQuery(name="SelectCountSMSByStatus",
        query="SELECT count(s) FROM SMS s where s.personMovil.person.id = :personId and s.status.id = :status and s.sentDate BETWEEN :from AND :to  "),
