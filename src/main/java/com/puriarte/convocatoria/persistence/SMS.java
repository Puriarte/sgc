...
@NamedQueries({
        ...
        query="SELECT s FROM SMS s " +
        		" where s.creationDate BETWEEN :from AND :to and ((:estado =0) or (s.status.id = :estado) )  "),
        ...
        query="SELECT s FROM SMS s where s.personMovil.id = :movilId and s.status.id = :status order by s.sentDate desc"),
